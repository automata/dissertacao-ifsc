...
%% - escrever melhor 2.3.1 e dar exemplos do gonzalez talvez
%% - rever :

%% x incluir tabela de atributos A_i
%% x referenciar todo o texto, parte de fundamentos principalmente
%% x revisar a notação (ver os livros costa e gonzales, se esta correto)
%% x melhorar a conclusão

%% x refazer figura 2.13 e 2.14, trocando v_i v_j v_k por p_...
%% ? revisar e traduzir todas as imagens
%% x terminar 2.4.3 Atributos de contorno e forma, ver o codigo fonte
%% x rever na ref a tabela 2.17 (atributos de haralick) e arrumar log e outros
%% termos
%% x atualizar a descrição da curvatura
%% x colocar medidas de media, ... jah no inicio (nocoes)
%% x rever nos scripts como posicionei a janela/escala das imagens capitulo 3
%% x figura 3.3, ver os parametros exatos de slic
%% x incluir detalhes para imagem no apêndice
%% x colocar os passos da geracao de pinturas e um ou dois exemplos no apendice
...
%%   \begin{center}
%%     \includegraphics[width=0.48\textwidth]{figs/brunelleschi.png}
%%   \end{center}
%%   \caption{Retratação do experimento de Brunelleschi que permitiu-lhe
%%     desenvolver a técnica de representação de profundidade pela perspectiva,
%%     onde todas as linhas de um objeto convergem a um só ponto, o ponto de fuga.~\cite{andersen}}
%%   \label{fig:pespectiva}
%% \end{wrapfigure}

% mostrar que a arte muda, que apresenta períodos, movimentos, escolas
% e artistas com características bem definidas. é por essa distinção
% e/ou similaridade de estilos/estéticas, que nosso estudo é possível.
...
Essa solução não foi suficiente para Cézanne nem seus contemporâneos
...
Na música, essa concepção de técnicas e afloramento de estilos é
...
A evolução das artes parece ser influenciada por essa solução de
...
É interessante levantar aqui algumas características sobre a história e estética
...
O uso da luz, contrastando rigidamente com o negro fundo, colabora para
...
\textbf{\emph{Frans Hals}}, Holanda, 1580?-1666. Ao mesmo tempo que o Barroco
...
De Vermeer à van Gogh, passam-se por volta de 150 anos, toda a Renascença, o
...
\textbf{\emph{Vincent van Gogh}}, Holanda, 1853-1890. Junto com Cezánne e
...
\textbf{\emph{Henri Matisse}}, França, 1869-1954. Foi o mais famoso pintor do
...
\textbf{\emph{Pablo Picasso}}, Espanha, 1881-1973. Estava interessado no
...
\textbf{\emph{Jackson Pollock}}, Estados Unidos, 1912-1956. Abandonou as imagens
...
A oposição de ideias está intimamente relacionada à dialética. Há grande
...
As imagens podem ainda ser representadas em escala de cinza, por uma matriz
...
Tendo uma vizinhança em torno de um pixel $I_{(i,j)}$ e uma função $f(x,y)$ que
representa tal pixel (e por conseguinte, a própria imagem), pode-se definir uma
função de processamento~\cite{gonzalez} da imagem como

\begin{equation}
  g(x,y) = T[f(x,y)]
\end{equation}

\noindent sendo $f(x,y)$ a imagem de entrada, $g(x,y)$ a imagem processada e $T$
é um operador sobre $f$, definido sobre alguma vizinhança-$N_V$ de
$(x,y)$. Dessa forma, o centro $(x,y)$ da sub-imagem que compreende os pixeis da
vizinhança-$N_V$ é movido de pixel em pixel, o que corresponde a alterar os
valores $f(x,y)$ do centro pelo valor obtido por $g(x,y)$. É possível usar
várias formas de subimagens (e.g.\ círculos ou elipses) mas subimagens quadradas
são mais comuns para vizinhanças pois são de fácil implementação.

O operador $T$ é essencial nesse tipo de processamento, pois é ele quem mapeia
os valores de $f$ em $g$. Uma operação de mapeamento muito comum para $T$ é
mapear o nível de cinza $r$ em $f$ para o nível de cinza $s$ em $g$, ou:

\begin{equation}
  s = T(r)
\end{equation} 

\noindent assim os níveis de cinza da imagem processada dependerá de alguma
operação definida em $T$. A equalização de uma imagem de entrada a partir de um
operador $T$ baseado no histograma de tons de cinza da imagem é um exemplo de
aplicação, discutido em detalhes na Subseção~\ref{sec:preproc}.
...
Durante todo este estudo, serão utilizados estes conceitos, e soma-se a eles
alguns outros, emprestados da Estatística. Portanto, são revisados aqui os momentos
...
%% TODO: talvez acrescentar uma figura exemplificando a equalização por histograma?
...
Para remover as regiões com áreas não representativas de uma região $R_i$ usa-se
...
Para este estudo foram utilizados 99 atributos, descritos nas próximas seções e
...
Portanto, a informação média associada à ocorrência do evento $E$ dados todos os
$a_j$ símbolos é:
...
Essas mesmas medidas, que foram aplicadas à representação da imagem original em
...
%% Ainda, o LDA quando usado em conjunto com
%% distribuição Gaussiana e regra de Bayes, torna-se um classificador. Detalhes
%% sobre esse algoritmo também são apresentados.
...
%% TODO: Como usar regra de Bayes para classificar.

%% TODO: Colocar figuras??
...
%% A Tabela X lista todos os atributos discutidos aqui e usados para caracterizar
%% cada pintura considerada.

%% TODO: discutir cada atributo e sua relevância? Aqui ou na próxima sec?
...
\section{Classificação e medidas de dialética, oposição e inovação}
\label{sec:medidas}

% retirei daqui as descrições analíticas da dialética, oposição e inovação
...
Detalhes interessantes surgem dessa projeção. A mais impressionante é a grande
...
No que tange a contra-dialética, mostrada na Tabela~\ref{tab:dialetica1} e
...
% TODO: Responder às questões levantadas na introdução.
...
A caracterização das pinturas pelo uso destes atributos tornou possível a
...
Embora insuficiente para esgotar todas as características encontradas em um
