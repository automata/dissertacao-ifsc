...
\end{verbatim}
  \item Para validar os resultados através do LDA:
\begin{verbatim}
  python valida3_r.py
\end{verbatim}
...
\centering
  ...
  \subfloat{%
    \includegraphics[width=.24\textwidth]{figs/thumbs/gogh/12b}}\hfill
  \subfloat{%
...
disponível em \\ \url{http://automata.cc/master-thesis} que apresenta a
gravação comentada do uso do aplicativo.
...
\begin{figure}[h!]
  ...
  \caption{Detalhe da figura ~\ref{fig:alternativa} do grupo de pinturas
    pertencentes exclusivamente à Jackson Pollock. Por este pintor ter
...
\chapter{Contribuições artísticas}
\label{ap:sifisc}

Além da análise de pinturas e sua interpretação, este estudo pôde
contribuir de maneira artística em algumas oportunidades aqui
apresentadas. Tendo como base o processamento de imagens
(principalmente as etapas de pré-processamento e segmentação)
realizado para as 240 pinturas usadas nesse estudo, foram geradas 240
novas \textit{pinturas}. Todas as 240 \textit{pinturas generativas}
estão disponíveis em
\url{http://www.flickr.com/photos/auto_mata/sets/72157634660390040/}. Nos
dias 30 de Setembro à 4 de Outubro de 2013, um conjunto destas
\textit{pinturas} foi aceito para exposição no espaço ``Obra
Artística'' da $3^a$ Semana Integrada do Instituto de Física de São
Carlos (SIFISC 3). Seguem os detalhes da obra, que também descreve o
algoritmo implementado para a geração das pinturas:
...
Todo o código-fonte assim como as \textit{pinturas} geradas, e suas versões para
...
\textit{remixadas} com base em novas imagens, acompanhavam cada \textit{pintura}. As
\textit{pinturas} expostas foram escolhidas através da consulta à preferência de pessoas
em uma \textit{thread} da lista de emails do
...
próprios autores deste estudo~\footnote{Em
  ...
  que pretende-se publicar na revista aberta e independente \textit{Quiosque}, em 2014.}

\begin{figure}[htb]
  \begin{center}
    \caption{Pintura generativa, criada a partir de tesselação de Delaunay da obra original de Frans Hals.}
  \label{fig:posterA}
...
\begin{figure}[htb]
  \begin{center}
    \caption{Pintura generativa, criada a partir de tesselação de Delaunay da obra original de Picasso.}
  \label{fig:posterB}
...
\begin{figure}[htb]
\centering 
  \caption{Fotos da exposição realizada no SIFISC 2013, no espaço
    ``Obra Artística'' com imagens geradas por algoritmo desenvolvido
...
Castro Rocha Junior, curadoria de Leticia Palaria e texto de Renato Fabbri e
integrantes do LabMacambira.sf.net. Uma prévia do conteúdo a ser
publicado pode ser visto na figura~\ref{fig:quiosque}.

\begin{figure}[htb]
  \begin{center}
    \caption{Prévia do conteúdo da publicação impressa intitulada
      ``Pinturas generativas \& Freakcoding'' prevista para 2014,
      através da Editora independente Quiosque.}
  \label{fig:quiosque}
